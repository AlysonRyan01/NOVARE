...
public static class InvoiceGatewayEndpoints
{
    public static void MapInvoiceGatewayEndpoints(this WebApplication app)
    {
        ...
        customersGroup.MapGet("/", async (
            ...
            {
                var result = await invoiceService.GetCustomersAsync();
        ...
        customersGroup.MapGet("/{id:guid}", async (
                ...
                Guid id) =>
            {
                var result = await invoiceService.GetCustomerByIdAsync(id);
                return result.IsSuccess
                    ? Results.Ok(result)
        ...
        customersGroup.MapPost("/", async (
            ...
            {
                var result = await invoiceService.CreateCustomerAsync(createCustomerDto);
                return result.IsSuccess
                    ? Results.Ok(result)
                    : Results.BadRequest(result);
            })
